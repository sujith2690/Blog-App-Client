...
import { getUserBlogs } from '../API/BlogRequest'
import Header from './Header'

const MyBlogs = () => {
    const [UserBlogs, setUserBlogs] = useState()
    const userId = '64b0fde29f6cad38d68dd803'
    const MyBlogs = async () => {
        const userBlogs = await getUserBlogs(userId)
        console.log(userBlogs.data.userBlogs.blogs, '----64b0fde29f6cad38d68dd803')
        const blogs = userBlogs.data.userBlogs.blogs
        setUserBlogs(blogs)
    }
    useEffect(() => {
        MyBlogs()
    ...
    return (
        <>
            <Header />
            <section className="dark:bg-gray-800 dark:text-gray-100 p-5">
